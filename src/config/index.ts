...
import { defaultConfig, type OrchestratorConfig } from './default.js';
 ...
 * 환경별 설정 오버라이드 가져오기
 */
function getEnvironmentConfig(env: Environment): Partial<OrchestratorConfig> {
  switch (env) {
    case 'production':
...
export const config: OrchestratorConfig = deepMerge(defaultConfig, envConfig);

/**
 * 설정 검증
 */
function validateConfig(cfg: OrchestratorConfig): void {
  // API 키 검증
  if (!cfg.llm.apiKey || cfg.llm.apiKey.trim() === '') {
    throw new Error(
      'ANTHROPIC_API_KEY is not set. Please set it in .env file or environment variable.'
    );
  }
